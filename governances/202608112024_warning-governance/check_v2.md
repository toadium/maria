...
- 实际执行：`moon check`（本项目为 MoonBit 项目，无 `cjpm.toml`，`cjpm` 不可用；`moon check` 已覆盖源码与测试代码的编译检查，等价于 build + test 编译阶段。`moon test --no-run` 参数不被 moon 工具支持）
...
─────╯
Failed with 28 warnings, 0 errors.
Error: failed when checking project
```

## 判定

- **ISSUES_FOUND**：编译输出中存在 28 个 warning